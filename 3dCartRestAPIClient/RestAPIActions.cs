...
namespace DCartRestAPIClient
{
    ...
    public class RestAPIActions 
    {
        ...
        public long RecordsOffset { get; set; }
        public string datestart { get; set; }
        public int invoicenumberstart { get; set; }

        public RestAPIActions()
        {
            datestart = "";
            RecordsOffset = 0;
            RecordsGetLimit = 100;
        ...
        public RecordInfo GetRecords<T>()
        {
            ...
            try
            {
                ...
                if (RecordsOffset > 0)
                {
                    sTypeWithLimit = sTypeWithLimit + "&offset="+ RecordsOffset;
                }
                if (datestart != "")
                {
                    sTypeWithLimit = sTypeWithLimit + "&datestart=" + datestart;
                }
                if (invoicenumberstart != 0)
                {
                    sTypeWithLimit = sTypeWithLimit + "&invoicenumberstart=" + invoicenumberstart.ToString();
                }
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    recordList = JsonConvert.DeserializeObject<List<T>>(receiveStream);
                }
                else
                {
